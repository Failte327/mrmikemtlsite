...
existing_tournaments = dbsession.exec_driver_sql(f"SELECT tournament_id FROM tournaments2026;").all()
...
for id in ids:
    matches_query = dbsession.exec_driver_sql(f"SELECT tournament_id FROM tournament_matches2026 WHERE tournament_id = '{id}';").all()
    if len(matches_query) == 0:
        ...
        for match in match_data:
            if match is not None:
                ...
                dbsession.exec_driver_sql(f"INSERT INTO tournament_matches2026 (participant_1_id, participant_2_id, participant_1_points, participant_2_points, tournament_id) VALUES ({participant_1_user_id}, {participant_2_user_id}, {participant_1_points}, {participant_2_points}, {id});")
                dbsession.commit()
...
#             dbsession.exec_driver_sql(f"UPDATE participants2026 SET user_id = {user_id} WHERE name = '{username}';")
#             dbsession.commit()
